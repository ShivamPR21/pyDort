    ...
    for log_id in log_ids:
        ...
        for cam_idx, camera_name in enumerate(RING_CAMERA_LIST + STEREO_CAMERA_LIST):
            ...
            for i, im_fpath in enumerate(cam_im_fpaths):
                ...
                img = imageio.v2.imread(im_fpath)[:, :, ::-1].copy()
                ...
                for label_idx, label in enumerate(labels):
                    ...
                    img = obj_rec.render_clip_frustum_cv2(
                        img, uv_cam.T[:, :3], planes.copy(), copy.deepcopy(camera_config), colors
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_num_images_to_render", default=5, type=int, help="number of images within which to render 3d cuboids"
    )
    parser.add_argument("--dataset_dir", type=str, required=True, help="path to the dataset folder")
    parser.add_argument("--labels_dir", type=str, required=True, help="path to the dataset folder")
    parser.add_argument(
        "--log_ids",
        type=str,
        required=True,
